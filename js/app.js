/* ============================================
   Social Structure Academy - Main Application
   ============================================ */
...
const App = {
    ...
    init() {
        ...
        this.buildSidebar();
        this.showDashboard();
        if (localStorage.getItem('social-structure-darkmode') === 'true') {
    ...
    loadProgress() {
        try {
            const saved = localStorage.getItem('social-structure-progress');
            if (saved) this.progress = JSON.parse(saved);
            const savedQuiz = localStorage.getItem('social-structure-quiz-results');
            if (savedQuiz) this.quizResults = JSON.parse(savedQuiz);
        } catch (e) { this.progress = {}; this.quizResults = {}; }
    },

    saveProgress() {
        localStorage.setItem('social-structure-progress', JSON.stringify(this.progress));
        localStorage.setItem('social-structure-quiz-results', JSON.stringify(this.quizResults));
    },
    ...
    showDashboard() {
        ...
        let html = `<div class="fade-in">
            <div class="dashboard-hero" style="background: linear-gradient(135deg, #4c1d95 0%, #6d28d9 50%, #7c3aed 100%);">
                <h2>🌐 社会構造アカデミーへようこそ</h2>
                <p>世の中の「見えないルール」を読み解き、社会構造を理解するための総合学習プラットフォームです。</p>
            </div>
        ...
        let html = `<div class="fade-in">
            <div class="module-header">
                <div class="module-breadcrumb">
                    ...
                    <a onclick="App.showModule(${level.modules[0].id})">Level ${level.id}: ${level.title}</a> &nbsp;/&nbsp; ${mod.title}
                </div>
            ...
            <div class="module-nav">
                ...
                <div style="display:flex;gap:12px;">
                    ${mod.quiz && mod.quiz.length > 0 ? `<button class="btn btn-primary btn-lg" onclick="App.startQuiz(${moduleId})">理解度チェック (${mod.quiz.length}問)</button>` : `<button class="btn btn-success btn-lg" onclick="App.completeModule(${moduleId}); App.goToNextModule(${moduleId});">完了して次へ</button>`}
                </div>
                <div>${nextMod ? `<button class="btn btn-outline" onclick="App.showModule(${nextMod.id})">${nextMod.title} →</button>` : ''}</div>
            </div>
        </div>`;
        container.innerHTML = html;
    ...
    showCompletionMessage() {
        ...
        modal.innerHTML = `
            ...
            <p>全モジュールを完了しました！<br>社会構造の「見えないルール」を理解する力が身につきました。<br>この知識を日常に活かし、構造の中で自由を広げていきましょう！</p>
    ...
    toggleDarkMode() {
        ...
        if (isDark) {
            ...
            localStorage.setItem('social-structure-darkmode', 'false');
        } else {
            document.documentElement.setAttribute('data-theme', 'dark');
            localStorage.setItem('social-structure-darkmode', 'true');
        }
    },
    ...
    resetProgress() {
        if (confirm('全ての学習進捗をリセットしますか？この操作は元に戻せません。')) {
            this.progress = {};
            this.quizResults = {};
            localStorage.removeItem('social-structure-progress');
            localStorage.removeItem('social-structure-quiz-results');
            this.buildSidebar();
            this.updateGlobalProgress();
            this.showDashboard();
        }
